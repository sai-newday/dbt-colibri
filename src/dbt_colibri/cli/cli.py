...
@cli.command("publish-artifacts")
@click.option(
    "--repo-name",
    type=str,
    required=True,
    help="Repository name to publish under artifacts/<repo-name>.",
)
@click.option(
    "--source-manifest",
    type=str,
    default="target/manifest.json",
    help="Path to the repo manifest.json generated by dbt.",
)
@click.option(
    "--source-catalog",
    type=str,
    default="target/catalog.json",
    help="Path to the repo catalog.json generated by dbt docs generate.",
)
@click.option(
    "--artifacts-root",
    type=str,
    default="artifacts",
    help="Directory that stores per-repo and master artifacts.",
)
@click.option(
    "--master-name",
    type=str,
    default="master",
    help="Name of the folder under artifacts/ used for merged outputs.",
)
@click.option(
    "--branch",
    type=str,
    default="artifacts-sync",
    help="Git branch to commit and push to.",
)
@click.option(
    "--commit-message",
    type=str,
...
def publish_artifacts_cmd(
    ...
    """Publish repo artifacts and regenerate the merged master manifest/catalog.

    The command copies the repo's manifest/catalog into artifacts/<repo-name>/,
    merges all available repo artifacts into artifacts/master/, and optionally
    commits/pushes the updated branch when run inside a git checkout.
    """
    if not os.path.exists(source_manifest):
        click.echo(f"❌ Manifest file not found at {source_manifest}")
        sys.exit(1)
    if not os.path.exists(source_catalog):
        click.echo(f"❌ Catalog file not found at {source_catalog}")
        sys.exit(1)

    artifacts_root_path = Path(artifacts_root)
    repo_artifact_dir = artifacts_root_path / repo_name
    master_artifact_dir = artifacts_root_path / master_name
    repo_artifact_dir.mkdir(parents=True, exist_ok=True)
    master_artifact_dir.mkdir(parents=True, exist_ok=True)

    shutil.copy2(source_manifest, repo_artifact_dir / "manifest.json")
    shutil.copy2(source_catalog, repo_artifact_dir / "catalog.json")

    project_artifacts = []
    for item in sorted(artifacts_root_path.iterdir(), key=lambda p: p.name):
        if not item.is_dir() or item.name.startswith(".") or item.name == master_name:
            continue

        manifest_path = item / "manifest.json"
        catalog_path = item / "catalog.json"
        if manifest_path.exists() and catalog_path.exists():
            project_artifacts.append((item.name, str(manifest_path), str(catalog_path)))

    if not project_artifacts:
        click.echo("❌ No published repo artifacts found to merge.")
        sys.exit(1)
    ...
    try:
        ...
        manifest_path, catalog_path = write_merged_artifacts(
            master_artifact_dir, merged_manifest, merged_catalog
        )
    except Exception as e:
        click.echo(f"❌ Error merging artifacts: {str(e)}")
        sys.exit(1)

    click.echo("✅ Published repo artifacts")
    click.echo(f"  📄 Repo manifest: {repo_artifact_dir / 'manifest.json'}")
    click.echo(f"  📄 Repo catalog:   {repo_artifact_dir / 'catalog.json'}")
    click.echo(f"  📄 Master manifest: {manifest_path}")
    click.echo(f"  📄 Master catalog:  {catalog_path}")
    if normalized_sources:
        click.echo(f"  🔁 Normalized cross-project sources: {normalized_sources}")

    if os.path.isdir(".git"):
        status_code, _, status_err = run_command(["git", "add", artifacts_root])
        if status_code != 0:
            click.echo(f"❌ Error staging artifacts: {status_err.strip()}")
            sys.exit(1)

        diff_code, _, diff_err = run_command(["git", "diff", "--cached", "--quiet", "--exit-code"])
        if diff_code == 0:
            click.echo("No artifact changes to commit.")
            sys.exit(0)
        if diff_code not in (0, 1):
            click.echo(f"❌ Error checking staged changes: {diff_err.strip()}")
            sys.exit(1)

        if not commit_message:
            commit_message = f"Update dbt artifacts for {repo_name}"

        commit_code, _, commit_err = run_command(["git", "commit", "-m", commit_message])
        if commit_code != 0:
            click.echo(f"❌ Error creating commit: {commit_err.strip()}")
            sys.exit(1)

        push_code, _, push_err = run_command(["git", "push", "origin", f"HEAD:{branch}"])
        if push_code != 0:
            click.echo(f"❌ Error pushing artifacts branch: {push_err.strip()}")
            sys.exit(1)

        click.echo(f"✅ Pushed artifacts branch: {branch}")
